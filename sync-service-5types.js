    ...
    async syncPackages() {
        ...
        try {
            ...
            for (const pkg of packages) {
                // Déterminer le type selon la nouvelle logique (5 types)
                let type = 'local'; // Par défaut: pays spécifique

                // 1. Si c'est une recharge
                if (pkg.supportTopUpType === 1 || pkg.name?.toLowerCase().includes('topup') || pkg.name?.toLowerCase().includes('top-up')) {
                    type = 'topup';
                }
                // 2. Si c'est unlimited (data illimitée)
                else if (pkg.name?.toLowerCase().includes('unlimited') || pkg.dataType === 2) {
                    type = 'unlimited';
                }
                // 3. Si c'est global/mondial
                else if (pkg.name?.toLowerCase().includes('global') || pkg.name?.toLowerCase().includes('world')) {
                    type = 'global';
                }
                // 4. Si c'est régional (plusieurs pays)
                else if (pkg.locationCode?.includes(',') || pkg.locationCode?.includes('-') ||
                    pkg.name?.toLowerCase().includes('europe') ||
                    pkg.name?.toLowerCase().includes('asia') ||
                    pkg.name?.toLowerCase().includes('middle east')) {
                    type = 'region';
                }
                // 5. Sinon c'est local (un seul pays)
